        ...
        if (empty($username) || empty($password)) {
            ...
            if (mysqli_num_rows($check) > 0) {
                ...
                if (mysqli_stmt_execute($stmt)) {
                    $message = "Barista account created successfully.";
                } else {
    ...
    if (isset($_POST['update_barista'])) {
        ...
        if (empty($username)) {
            ...
            if (mysqli_num_rows($check) > 0) {
                ...
                // Scenario A: Update Username AND Password
                if (!empty($password)) {
                    if ($password !== $confirm_pass) {
                        ...
                        $stmt = mysqli_prepare($conn, "UPDATE users SET username=?, password=? WHERE id=? AND role='barista'");
                        mysqli_stmt_bind_param($stmt, "ssi", $username, $hashed_password, $id);
                        if (mysqli_stmt_execute($stmt)) {
                            $message = "Account updated (Password changed).";
                            $edit_mode = false; // Reset form
                        }
                    }
                } 
                // Scenario B: Update Username ONLY
                else {
                    $stmt = mysqli_prepare($conn, "UPDATE users SET username=? WHERE id=? AND role='barista'");
...
if (isset($_GET['delete'])) {
    ...
    $stmt = mysqli_prepare($conn, "DELETE FROM users WHERE id=? AND role='barista'");
    mysqli_stmt_bind_param($stmt, "i", $id);
    if (mysqli_stmt_execute($stmt)) {
        $message = "Barista account deleted.";
    } else {
...
if (isset($_GET['edit'])) {
    ...
    $res = mysqli_query($conn, "SELECT * FROM users WHERE id=$edit_id AND role='barista'");
...
<head>
    ...
    <title>Manage Baristas | Hossana Cafe</title>
    ...
    <nav class="sidebar" id="sidebar">
        ...
        <div class="nav-links">
            ...
            <div id="userManage" class="submenu-container show">
                ...
                <a href="add_barista.php" class="nav-item sub-link active">Baristas</a>
    ...
    <main class="main-content">
        <header class="top-header">
            <div style="display:flex; align-items:center;">
                ...
                <div class="page-title">Manage Baristas</div>
            </div>
        ...
        <div class="content-scroll">
            ...
            <div class="split-container">
                <div class="card-panel">
                    <div class="card-header"><?php echo $edit_mode ? "Edit Barista Account" : "Add New Barista"; ?></div>
                    <form action="add_barista.php" method="post">
                        ...
                        <button type="submit" name="<?php echo $edit_mode ? 'update_barista' : 'add_barista'; ?>" class="btn-submit"><?php echo $edit_mode ? "Update Account" : "Create Account"; ?></button>
                        <?php if($edit_mode): ?><a href="add_barista.php" class="btn-cancel">Cancel Edit</a><?php endif; ?>
                    </form>
                </div>
                <div class="card-panel">
                    <div class="card-header">Existing Barista Accounts</div>
                    <div class="table-wrapper">
                        <table>
                            <thead><tr><th>ID</th><th>Username</th><th style="text-align:right;">Actions</th></tr></thead>
                            <tbody>
                                <?php 
                                $sql = "SELECT id, username FROM users WHERE role='barista' ORDER BY id DESC";
                                ...
                                <tr>
                                    ...
                                    <td style="text-align:right;">
                                        <a href="add_barista.php?edit=<?php echo $row['id']; ?>" class="btn-small btn-blue">Edit</a>
                                        <a href="add_barista.php?delete=<?php echo $row['id']; ?>" class="btn-small btn-red" onclick="return confirm('Delete this barista account?');">Delete</a>
                                    </td>
                                </tr>
                                <?php endwhile; else: ?>
                                    <tr><td colspan="3" style="text-align:center; padding:40px; color:#999;">No barista accounts found.</td></tr>
                                <?php endif; ?>
                            </tbody>
                        </table>
                    </div>
